...
class Qxw(Q):
    ...
    def get_rels(self, prop, N):
        """TODO: EXPLICITLY ignores t, d, pvt, ci, pi
        """
        d, pvt, s = [np.array([])]*3
        raise NotImplementedError

        x_array, pi_array, y_array = data.cell_data(self.xwcell)

        return np.hstack((x_array, pi_array)), np.vstack(y_array)
...
#                 ax.axvline(i, linestyle='-', color='k')
    ...
    def get_rels(self, prop, data):
        """TODO: EXPLICITLY ignores t, d, pvt, ci, pi
        """
        x_array, y_array = data.cell_xydata(self.xcell.cell)

        assert(x_array.size != 0)
        assert(not np.any(prop.final_cons.sat(x_array)))
        assert(not np.any(prop.final_cons.sat(y_array)))
